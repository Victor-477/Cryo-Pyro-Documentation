...
(function () {
  ...
  var pages = [
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "  <div class=\"stage\"><span class=\"st-tag\">3 · run</span><h4>Pyro <code>.pyro</code></h4><p>Custom bytecode running on the Pyro VM (in Go).</p></div>",
        "</div>",
        "",
        ...
        "",
        "<div class=\"card-grid\">",
        "  <a class=\"card\" href=\"#/instalacao\"><span class=\"card-emoji\">⚙️</span><h4>Installation</h4><p>What you need: Python and (optionally) Go, gcc/MinGW.</p></a>",
        "  <a class=\"card\" href=\"#/inicio-rapido\"><span class=\"card-emoji\">🚀</span><h4>Quick start</h4><p>Compile and run your first program in a minute.</p></a>",
        "  <a class=\"card\" href=\"#/sintaxe\"><span class=\"card-emoji\">📖</span><h4>The language</h4><p>Types, functions, control flow and operators.</p></a>",
        "  <a class=\"card\" href=\"#/agent\"><span class=\"card-emoji\">🤖</span><h4>AI agents</h4><p>schema, tools and the native agent loop.</p></a>",
        "</div>",
        "",
        "## A taste",
        "",
        ...
        "Fatura f = llm(\"gpt-x\", \"Extract the invoice: \" + texto) as Fatura;",
        "",
        ...
        "| **Pyro** | Target language (bytecode + VM) | `pyro/vm/main.go`, `PYRO_BYTECODE.md` |",
        "",
        "> **Language version:** v0.9. The **go** backend is the most complete (structs, arrays, maps, JSON, optionals, LLM); the **pyro** backend covers the core and keeps growing."
      )
    },
    {
      ...
      body: md(
        ...
        "",
        "```text",
        "  write             compile (Burnout)              run (Pyro VM)",
        " .cryo   ─────►   lexer → parser → AST → codegen   ─────►   .pyro + VM   ─────►  output",
        "                            │",
        "             alternative targets: Go (.go) · C (.c) · x86-64 (.s)",
        "```",
        "",
        ...
        "5. **Toolchain** — the Pyro VM (Go) runs the `.pyro`; or `go`/`gcc` builds the binary for the other targets.",
        "",
        "## Components on disk",
        "",
        "```text",
        "Pyro_Cryo/",
        "├── cryo/       # CRYO    — the source language: lexer, parser, AST, analysis + examples/",
        "├── burnout/    # Burnout — the compiler: cryoc.py, compiler.py, codegens, runtime, tests",
        "├── pyro/       # PYRO    — the target language: Pyro VM (Go) + bytecode spec",
        ...
        "",
        "## Five targets, one front-end",
        "",
        "The same AST feeds five code generators. You pick with `--backend`:",
        "",
        "| Backend | Output | For what |",
        "|---|---|---|",
        "| `go` (default) | `.go` → Go binary | Full language: LLM, maps, JSON, concurrency |",
        "| `pyro` | `.pyro` (bytecode) → VM | The custom target language; compact and portable |",
    ...
    },
    {
      ...
      body: md(
        ...
        "| Run the **pyro** target (bytecode + VM) | **Go 1.18+** (builds the VM once) |",
        "| Run the **node** target (JavaScript) | **Node.js** |",
        "| Run the **c** / **asm** targets | **gcc** on `PATH` (MinGW on Windows) |",
        "",
        "> The Python compiler **always** generates the source file (`.go`, `.pyro`, `.c` or `.s`). The toolchain is only needed to build/run the binary.",
        "",
        "## Getting the project",
        "",
        "```bash",
        ...
        "```",
        "",
        "## Windows: ready-made build script (c/asm backends)",
        "",
        "To build directly on Windows with MinGW, use the script — it locates gcc (including common MSYS2/TDM/WinLibs installs), generates the Win64 assembly and builds the `.exe`:",
        "",
        "```bash",
        "REM cmd / Explorer",
    ...
    {
      slug: "inicio-rapido", group: "Getting Started", title: "Quick start",
      ...
      body: md(
        ...
        "",
        "```bash",
        "python burnout/cryoc.py ola.cryo --tokens     # print the tokens",
        "python burnout/cryoc.py ola.cryo --ast        # print the AST",
        "python burnout/cryoc.py ola.cryo --audit      # security audit",
        "python burnout/cryoc.py ola.cryo --backend pyro --dis   # disassemble the bytecode",
        "```",
        "",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "fn dobro(int x) -> int ={    // body opens with ={",
        "    return x * 2;",
        "}",
        "```"
      )
    },
    {
      ...
      body: md(
        ...
        "```cryo",
        "int    qtd   = 2;",
        "number frete = 12.50 + qtd * 6.00;   // qtd becomes float -> 24.50",
        "```"
      )
    },
    {
      ...
      body: md(
        ...
        "    if (sku == \"SKU-1\") { return new Produto { nome: \"Headphones\", preco: 129.90 }; }",
        "    return new Produto { nome: \"Unknown\", preco: 0.0 };",
        "}",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "fn(int) -> int        umParametro;",
        "fn(int, int) -> number  dois;",
        "fn(string) -> void    efeito;",
        "```",
        "",
        "## Lambdas",
        "",
        ...
        "fn(int)->int fatN = (int n) => {           // block body",
        "    int acc = 1;",
        ...
        "print(aplicar((int x) => x + 100, 1));   // 101 (lambda inline)",
        "```",
        "",
        ...
        "}",
        "",
        "int[] triplos = mapear([1, 2, 3, 4], (int x) => x * 3);   // [3, 6, 9, 12]",
        "```",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "| Import cycle (`a -> b -> a`) | `[Erro de Módulo] import cycle detected: ...` |",
        "| Name clash between files | `[Erro de Módulo] duplicate declaration '...': defined in X and in Y` |",
        "| Missing file | `[Erro de Módulo] module not found` |",
        "",
        "## Via the API",
        "",
        "```python",
    ...
    },
    {
      ...
      lead: "`if`/`else`, `while`, `do/while`, the two kinds of `for`, `switch` and `break`/`continue`.",
      body: md(
        "## if / else",
        "",
        "```cryo",
    ...
    },
    {
      ...
      body: md(
        ...
        "```cryo",
        "string leitura = null;",
        "string resultado = leitura ?? \"no reading\";   // \"no reading\"",
        "```",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "    ativo: true",
        "};",
        "string nome = s1.nome;   // \"Temperature\"",
        "```",
        "",
        ...
        "    switch (nv) {",
        "        case Nivel_ALTO:  return \"HIGH\";",
        "        case Nivel_MEDIO: return \"MEDIUM\";",
        "        default:          return \"LOW\";",
        "    }",
        "}",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "    \"SKU-3\": 5.00",
        "};",
        "precos[\"SKU-4\"] = 99.90;      // insert/update",
        ...
        "number final = achado ?? 0.0;    // fallback if missing",
        "```"
      )
    },
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "number final = achado ?? 0.0;       // use 0.0 if achado is null",
        "string txt   = leitura ?? \"empty\";",
        "```",
        "",
        "## Comparing with null",
        "",
        ...
        "",
        "```cryo",
        "number valor = achado!;   // assumes achado != null",
        "```",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "Produto p = new Produto { sku: \"SKU-2\", nome: \"Pro Plan\", preco: 49.90, ativo: true };",
        ...
        "",
        "string entrada = \"{\\\"ok\\\": true, \\\"mensagem\\\": \\\"created\\\", \\\"total\\\": 4}\";",
        "RespostaAPI r = json_decode(entrada) as RespostaAPI;",
        "",
        "print(r.ok);         // true",
        "print(r.mensagem);   // created",
        "```",
        "",
        "## Relation to LLM",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "assert(fatorial(10) == 3628800, \"factorial incorrect\");",
        "assert(empacotado == 0x1234);   // message is optional",
        "```",
        "",
        "## Always-on safety",
        "",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "}",
        "",
        "future<int> a = spawn tarefa(6);   // fire off",
        ...
        "future<int>[] pendentes = [];",
        "for (int id in ids) {",
        "    pendentes.push(spawn tarefa(id));   // fire all at once",
        "}",
        "int soma = 0;",
        "for (future<int> f in pendentes) {",
        "    soma += await f;                    // collect the results",
        "}",
        "```",
        "",
        "## sleep",
    ...
    },
    {
      ...
      body: md(
        ...
        "}",
        "",
        "future<string> f1 = spawn baixar(\"https://a.example.com\");",
        "future<string> f2 = spawn baixar(\"https://b.example.com\");",
        "string r1 = await f1;",
        "string r2 = await f2;",
        "```",
        "",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "string texto = \"Customer Ana, total 149.90, items: pro plan\";",
        "Fatura f = llm(\"gpt-x\", \"Extract the invoice: \" + texto) as Fatura;",
        "print(f.cliente);",
        "print(f.total);",
        "```",
        ...
        "",
        "```cryo",
        "string resposta = llm(\"gpt-x\", \"Summarize in one sentence: Cryo compiles to Pyro\");",
        "```",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "}",
        "",
        "string resposta = agent(\"gpt-x\", \"What is the price of SKU-1 with a 10% discount?\");",
        "print(resposta);",
        "```",
        "",
        "The model chains `buscar_preco` → `aplicar_desconto` → final answer on its own.",
        "",
        "## Configurable agent",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "```cryo",
        "string saida = pyro_exec(\"echo hello-from-the-machine\");",
        "int agora    = pyro_time();",
        "print(agora > 0);",
        "```",
        "",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "| Option | Effect |",
        "|---|---|",
        "| `--backend {auto,go,c,asm,pyro,node}` | Choose the generator; `auto` picks the best for the program (default: `go`) |",
    ...
    },
    {
      ...
      lead: "Five code generators from the same AST. Each covers a subset of the language.",
      body: md(
        "## Coverage matrix",
        "",
        "| Feature | Go (base) | Node | C | asm | Pyro |",
        "|---|:---:|:---:|:---:|:---:|:---:|",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "Each foreign language must be enabled with `import >Lang<`. A `>Lang( ... )` block is only accepted if `Lang` was imported — otherwise the compiler rejects it with `[Erro Estrangeiro]`, **before** generating any code.",
        "",
        "```cryo",
        ...
        "",
        "```text",
        "[Erro Estrangeiro] foreign block >C( ... ) used without importing the",
        ...
        "",
        "```text",
        "[Erro Estrangeiro] library >math< is ambiguous: multiple imported",
    ...
    },
    {
      ...
      body: md(
        ...
        "[Erro Semântico] semantic analysis found 2 problem(s):",
        "  - Line 4: undeclared variable 'total'",
        "  - Line 7: function 'somar' expects 2 argument(s), got 1",
        "```",
        "",
        "Errors are **accumulated** and reported together (with the line), rather than stopping at the first. Type names (struct/enum/schema) and enum members (`Nivel_ALTO`) are recognized; functions may be called before they are declared (mutual recursion and forward references work).",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "int rapido = 0;",
        "unsafe {",
        "    rapido = soma * 2;   // no overflow check here",
        "}",
        "```",
        "",
        "## Static audit (--audit)",
        ...
        "",
        "```text",
        "[Cryo Seguranca] Sandbox: pyro_exec bloqueado por política de sandbox",
        "```",
        "",
        ...
        "[Auditoria] backend 'c' does not cover features [map].",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "",
        "```text",
        "[Pyro VM] [Cryo Seguranca] DivisaoPorZero: integer division",
    ...
    },
    {
      ...
      body: md(
        ...
        "| `LEN`/`APPEND`/`HAS`/`KEYS` | 64–67 | — | container operations |",
        ...
        "",
        "The `NATIVE` instruction consumes `argc` arguments from the stack and pushes the result. The id table is mirrored between the generator (`NATIVES` in `codegen_pyro.py`) and the VM (`native()` in `vm/main.go`):",
        "",
        ...
        "| | | | | 27 | `write_bytes` |",
        "",
        "Enums generate no code: each member (`Nivel_ALTO`) becomes an integer constant at compile time. `try`/`catch`/`throw` use the `TRYPUSH`/`TRYPOP`/`THROW` instructions; `??` and `x!` use `COALESCE`/`UNWRAP`. Literal global constants are inlined. `write_bytes(path, int[])` writes raw bytes to a file (gated by the sandbox) — the binary output that lets a program on the VM emit a `.pyro`.",
        "",
        "## Bytecode optimizer",
        "",
    ...
    },
    {
      ...
      lead: "The Pyro VM (in Go) loads, decodes and executes the `.pyro`. Typing is dynamic at runtime.",
      body: md(
        "## Dynamic typing",
        "",
        ...
        "## Security in the VM",
        "",
        "- Integer `DIV`/`MOD` by zero **abort** (`[Cryo Seguranca] DivisaoPorZero`).",
        ...
        "- **VM:** `pyro/vm/main.go` (load, decode and execute).",
    ...
    },
    {
      ...
      body: md(
        ...
        "- **WASM** backend; `spawn`/`await` on the Pyro VM",
        ...
        "",
        "## Phase 9 — Self-hosted Pyro (compilation independent of other languages) — planned",
        "",
        "Today the Pyro route depends on **Python** (the compiler) and **Go** (the VM). This phase removes both, turning Pyro into a closed system that compiles and runs itself — shippable as a single binary.",
        "",
        "| # | Item | Description |",
        "|---|---|---|",
        ...
        "| 9.3 | 🚧 **Cryo→Pyro compiler written in Cryo** (staged) | Reimplement lexer + parser + analysis + codegen in Cryo, compiled to `.pyro` and run on the VM — drops the **Python** dependency for compiling. **Stage 1 ✅ — Lexer** (`cryo/selfhost/lexer.cryo`) runs on the Pyro VM and emits a token stream byte-identical to the reference lexer (`test_selfhost.py`, 4/4). **Stages 1–3 ✅** — lexer (token-identical), parser (AST-identical) and **codegen** (`cryo/selfhost/codegen.cryo`): a single-pass Cryo compiler that emits a valid `.pyro` (PYRO v2 header, constant pool, function table, opcodes) and writes it with `write_bytes`, for an integer subset (`int` decls, assignment, `print`, `+ - * / %` with precedence, unary `-`, parens), comparisons (`== != < > <= >=`), **control flow** (`if`/`else`/`else if` and `while` via `JMP`/`JMPF` i32 with backpatching) and **user functions** (function table, `CALL`, params as slots, `return`, **recursion** — function names in the pool via a `charCode`/ASCII table). The generated `.pyro` **runs on the VM with output identical to the reference compiler** (`test_selfhost.py`, 14/14: arithmetic + flow + functions incl. recursive `fib`) — a self-hosting proof: compiled and run inside its own ecosystem, no Python. Remaining: widen the subset (strings/floats in the pool, `&&`/`||`, structs/arrays) toward full `codegen_pyro` parity; bootstrapping uses the Python `burnout` once |",
        ...
        "| 9.6 | **Single-binary distribution** | One `pyro` executable that compiles (`pyro build`) and runs (`pyro run`) with no Python/Go installed |",
        "",
        "Sequence: **9.1 → 9.2** free execution from Go; **9.3 → 9.4** free compilation from Python (with a fixpoint proof); **9.5 → 9.6** deliver the native route and the single binary. By the end, Pyro is a self-sufficient target — written, compiled and executed inside its own ecosystem.",
        "",
        "## Principles",
        "",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "| `arr.push(v)` | appends to the array |",
        "",
        "## JSON (go / node / pyro)",
        "",
        "| Builtin | Description |",
    ...
    {
      slug: "palavras-chave", group: "Reference", title: "Keywords",
    ...
    },
    {
      ...
      lead: "Ready-made programs under `cryo/examples/`, from the core to AI agents.",
      body: md(
        ...
        "| `example_modules.cryo` + `lib_geometria.cryo` | **modules** (`import \"file.cryo\"`) + interpolation | pyro/go/node |",
  ...
  var groups = [
    { title: "Getting Started", pages: ["introducao", "arquitetura", "instalacao", "inicio-rapido"] },
    ...
    { title: "Reference", pages: ["builtins", "palavras-chave", "exemplos"] }
  ];
