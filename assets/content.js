  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| 10.4 | **[Extended standard library](#/builtins)** — first batch shipped: `clamp`/`sign`/`gcd`/`hypot` and `starts_with`/`ends_with`/`repeat`, as natives with go/node/pyro parity (Go VM == C VM verified). Still to come: string `pad`, `time`, `random` | 🚧 |",
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "",
        "> `sort`/`reverse`/`slice` are **non-mutating** — they return a fresh array and leave the original untouched. The function-taking collection methods (`map`/`filter`/`reduce`) arrive with [VM closures (10.6)](#/roadmap).",
        "",
        "## JSON (go / node / pyro)",
        "",
        "| Builtin | Description |",
