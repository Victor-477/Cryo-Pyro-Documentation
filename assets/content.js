  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| | | | | 26 | `sleep` |",
        "",
        "Enums generate no code: each member (`Nivel_ALTO`) becomes an integer constant at compile time. `try`/`catch`/`throw` use the `TRYPUSH`/`TRYPOP`/`THROW` instructions; `??` and `x!` use `COALESCE`/`UNWRAP`. Literal global constants are inlined.",
        "",
        "## Bytecode optimizer",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "| 8.2 | **Enums with data + pattern matching** (`enum Result { Ok(int), Err(string) }`, `match`) | Algebraic data types — model states without `null` |",
        "| 8.3 | **Error propagation `?`** (early-return on `Result`/optional) | Error ergonomics without verbose `try/catch` |",
        ...
        "| # | Item | Description |",
        "|---|---|---|",
        "| 9.1 | **Pyro VM in C** | Rewrite the VM (today [`pyro/vm/main.go`](#/pyro-vm)) as portable single-file C, built by the same gcc/MinGW toolchain the c/asm backends already use — drops the **Go** dependency for running `.pyro` (output parity checked against the Go VM) |",
