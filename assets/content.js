  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| 9.3 | 🚧 **Cryo→Pyro compiler written in Cryo** (staged) | Reimplement lexer + parser + analysis + codegen in Cryo, compiled to `.pyro` and run on the VM — drops the **Python** dependency for compiling. **Stage 1 ✅ — Lexer** (`cryo/selfhost/lexer.cryo`) runs on the Pyro VM and emits a token stream byte-identical to the reference lexer (`test_selfhost.py`, 4/4). Binary-write primitive ✅ — `write_bytes(path, int[])` (NATIVE 27) on both VMs and the go backend (sandbox-gated) lets a program on the VM emit a `.pyro`. Remaining: parser and codegen in Cryo; bootstrapping uses the Python `burnout` once to compile the Cryo compiler |",
