...
(function () {
  ...
  var pages = [
    ...
    // =========================================================
    {
      ...
      body: md(
        ...
        "| `--sandbox` | pyro backend: bakes a sandbox policy into the `.pyro` (the VM refuses network/machine natives) |",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "## Runtime sandbox (Pyro VM)",
        "",
        "The static audit is advisory. The **sandbox** is runtime enforcement: the Pyro VM **refuses** network/machine natives, failing closed instead of reaching out. Compile with `--sandbox` and the policy is baked into the `.pyro` (format flag bit2) — the artifact stays sandboxed wherever it runs:",
        "",
        "```bash",
        ...
        "",
        "```text",
        "[Pyro VM] [Cryo Seguranca] Sandbox: http_get() bloqueado por política de sandbox",
        "  stack trace (most recent first):",
        "    at main (line 2)",
        "```",
        "",
        "You can also sandbox an **existing** `.pyro` at run time — without recompiling — by setting `PYRO_SANDBOX=1` in the environment. It only ever tightens: it can turn the sandbox on, never off (an artifact compiled with `--sandbox` stays sandboxed regardless of the variable).",
        "",
        "| Mechanism | Scope |",
        "|---|---|",
        "| `cryoc … --sandbox` | bakes flag bit2 into the `.pyro` (permanent for that artifact) |",
        "| `PYRO_SANDBOX=1` (env) | runtime policy over any `.pyro`, applied by the VM |",
        "",
        "> Today the VM gates `http_get`/`http_post` (its network surface); `sleep` stays allowed. The machine-access builtins (`pyro_exec`, `pyro_write_file`, `pyro_open`, …) live only on the **go** backend, which is outside the VM sandbox — for untrusted code, prefer the `pyro` target with `--sandbox`.",
        "",
        "### Backend suggestion",
        "",
    ...
    },
    {
      ...
      body: md(
        ...
        "",
        "- ✅ **[VM sandbox](#/seguranca)** (`--sandbox`) — the Pyro VM refuses network/machine natives; policy baked into the `.pyro` (flag bit2) or set at runtime with `PYRO_SANDBOX=1`",
        "- **WASM** backend; `spawn`/`await` on the Pyro VM",
        "- **Generics**, interfaces/traits and pattern matching in Cryo",
  ...
  var groups = [
    ...
    { title: "The Cryo Language", pages: ["sintaxe", "tipos", "funcoes", "modulos", "controle-de-fluxo", "operadores", "structs-enums", "arrays", "mapas", "opcionais", "json", "erros"] },
