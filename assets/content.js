  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| 9.3 | 🚧 **Cryo→Pyro compiler written in Cryo** (staged) | Reimplement lexer + parser + analysis + codegen in Cryo, compiled to `.pyro` and run on the VM — drops the **Python** dependency for compiling. **Stage 1 ✅ — Lexer** (`cryo/selfhost/lexer.cryo`) runs on the Pyro VM and emits a token stream byte-identical to the reference lexer (`test_selfhost.py`, 4/4). **Stage 2 ✅ — Parser** (`cryo/selfhost/parser.cryo`): recursive-descent with the same precedence as the reference parser; runs on the VM and yields an AST (S-expression) identical to `cryo/parser.py` on representative sources (`test_selfhost.py`, 8/8). Binary-write primitive ✅ — `write_bytes(path, int[])` (NATIVE 27) on both VMs and the go backend (sandbox-gated). Remaining: stage 3 (codegen in Cryo → `.pyro` bytes, written with `write_bytes`); bootstrapping uses the Python `burnout` once |",
