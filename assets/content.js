  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| 9.3 | 🚧 **Cryo→Pyro compiler written in Cryo** (staged) | Reimplement lexer + parser + analysis + codegen in Cryo, compiled to `.pyro` and run on the VM — drops the **Python** dependency for compiling. **Stage 1 ✅ — Lexer** (`cryo/selfhost/lexer.cryo`) runs on the Pyro VM and emits a token stream byte-identical to the reference lexer (`test_selfhost.py`, 4/4). **Stages 1–3 ✅** — lexer (token-identical), parser (AST-identical) and **codegen** (`cryo/selfhost/codegen.cryo`): a single-pass Cryo compiler that emits a valid `.pyro` (PYRO v2 header, constant pool, function table, opcodes) and writes it with `write_bytes`, for an integer subset (`int` decls, assignment, `print`, `+ - * / %` with precedence, unary `-`, parens), comparisons (`== != < > <= >=`) and **control flow** — `if`/`else`/`else if` and `while` via `JMP`/`JMPF` (i32) with label backpatching. The generated `.pyro` **runs on the VM with output identical to the reference compiler** (`test_selfhost.py`, 12/12: arithmetic + flow) — a self-hosting proof: compiled and run inside its own ecosystem, no Python. Remaining: widen the subset (user functions with `CALL`, strings/floats in the pool, `&&`/`||`) toward full `codegen_pyro` parity; bootstrapping uses the Python `burnout` once |",
