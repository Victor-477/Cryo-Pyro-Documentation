  ...
  var pages = [
    ...
    },
    {
      ...
      body: md(
        ...
        "| 11.3 | **Richer string formatting** — `${value:.2f}`, alignment, separators; today an aligned table needs manual padding |",
        "| 11.4 | **Error ergonomics** — `?` on a `Result` shape, `match` guards, a default/`or_else` form |",
        "| 11.5 | **Iteration protocol** — `for (k, v in map)` directly, and user types made iterable |",
        "",
        ...
        "| 11.15 | **Extend the audit** — unvalidated deserialization, unbounded allocation from input, TOCTOU on paths |",
        "",
        "### Track D — the LLM layer: control and better answers",
        "",
        "| Item | Description |",
        "|---|---|",
        "| 11.16 | **Generation controls** — `temperature`, `max_tokens`, `stop`, `seed`, `timeout`; none are reachable from Cryo today, and `seed` is what makes an LLM-using program testable |",
        "| 11.17 | **Streaming** — tokens as they arrive, exposed as an iterator |",
        "| 11.18 | **Validated structured output with repair** — a malformed object should trigger a bounded re-ask carrying the validation error, not an abort. The biggest quality win available: most bad answers are schema violations, not reasoning failures |",
        "| 11.19 | **Resilience** — retries with backoff, per-call timeouts, and a typed failure you can `match` on |",
        "| 11.20 | **Agent loop upgrades** — parallel tool calls, tool errors as values, a step budget, and context-window management |",
        "",
        "### Track E — Burnout: optimization and compiler resources",
        "",
        "| Item | Description |",
        "|---|---|",
        "| 11.21 | **Beyond peephole** — constant and copy propagation, dead-store elimination, inlining small leaf functions |",
