...
| 12.9 | ✅ **[A payload-less enum member as a value](#/structs-enums)** — `E e = A;` and `Status.ATIVO` work on all four backends, resolved at compile time, with a variable of the same name still shadowing the member and `p.RED` still a field read |
